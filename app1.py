...
if df is not None:
    ...
    for i in range(len(sorted_df)):
        ...
        if prev_grade and row['투자등급'] != prev_grade:
            st.markdown("<hr style='margin:4px 0 4px 0;'>", unsafe_allow_html=True)
    ...
    ).properties(title="투자등급 분포")
    st.altair_chart(chart1, use_container_width=True)

    # 차트 2: PER vs 상승여력 vs 배당률
    ...
    color_encoding = alt.Color('투자등급', scale=alt.Scale(
        domain=['🔥🔥🔥 초적극 매수', '🔥🔥 적극 매수', '🔥 매수', '👀 관망'],
        range=['salmon', 'LightPink', 'LightSkyBlue', 'dodgerBlue']
    ))
    ...
    ).interactive().properties(title="💡 PER vs 상승여력 vs 배당률")
    ...
    chart3 = alt.Chart(df).mark_bar().encode(
        ...
        y=alt.Y('종목', sort='x'),
        color=alt.Color('고점대비 (%)', scale=alt.Scale(scheme='redyellowblue'), legend=alt.Legend(title='고점대비 (%)')),
        tooltip=['종목', '고점대비 (%)']
    ).properties(title='📉 고점대비 하락률')
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        df.to_excel(writer, index=False, sheet_name='투자분석')
